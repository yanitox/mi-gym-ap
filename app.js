...
function mostrarVista(v, el) {
    ...
    if(v === 'profile') { updatePesoChart(); updateChartRendimiento(); updateCardioChart(); }
}

function abrirModalCrear() {
    const nombre = prompt("Nombre de la rutina:");
    const ejStr = prompt("Ejercicios y Series (ej: Press-3, Remo-4):");
...
window.renderHome = function() {
    const list = document.getElementById('lista-rutinas');
    if (list) {
        list.innerHTML = window.db.rutinas.map((r, i) => `
    ...
    const hist = document.getElementById('historial-entrenos');
    if (hist) {
        hist.innerHTML = window.db.historial.slice(-5).reverse().map((h, i) => {
    ...
    const sel = document.getElementById('select-ejercicio-grafica');
    if (sel) {
        const todosEjs = [...new Set(window.db.historial.flatMap(h => h.ejercicios.map(e => e.nome)))];
        sel.innerHTML = todosEjs.map(e => `<option value="${e}">${e}</option>`).join('');
    }
}
    ...
    cont.innerHTML = r.ejercicios.map((ej, i) => `
        <div class="card" id="ej-${i}">
            ...
            <table class="train-table">
                ...
                <tbody id="tb-${i}">${Array.from({length: ej.defaultSeries}, (_, s) => createRow(s+1)).join('')}</tbody>
            </table>
...
function addRow(i, t) { document.getElementById(`tb-${i}`).insertAdjacentHTML('beforeend', createRow(0, t)); }

function finalizarEntreno() {
    if(!confirm("¿Finalizar y guardar?")) return;
    const ejs = [];
    document.querySelectorAll('[id^="ej-"]').forEach(el => {
        const series = [];
        el.querySelectorAll('tbody tr').forEach(tr => {
            series.push({ n: tr.cells[0].innerText, kg: parseFloat(tr.querySelector('.k').value) || 0, reps: parseInt(tr.querySelector('.r').value) || 0 });
        });
        ejs.push({ nome: el.querySelector('h3').innerText, nota: el.querySelector('textarea').value, series });
    });
    window.db.historial.push({ ...sesionActiva, ejercicios: ejs, cardio: { min: parseFloat(document.getElementById('cardio-min').value) || 0, kcal: parseFloat(document.getElementById('cardio-kcal').value) || 0 } });
    save(); mostrarVista('home');
}

function updatePesoChart() {
    const ctx = document.getElementById('graficaPeso').getContext('2d');
    if(chartPeso) chartPeso.destroy();
    chartPeso = new Chart(ctx, { type: 'line', data: { labels: window.db.medidas.map(m => m.fecha).reverse(), datasets: [{ label: 'Peso Corporal (kg)', data: window.db.medidas.map(m => m.valor).reverse(), borderColor: '#0A84FF', tension: 0.3 }] }, options: { plugins: { legend: { display: false } } } });
}
...
function updateCardioChart() {
    ...
    chartCardio = new Chart(ctx, { type: 'bar', data: { labels: dataH.map(h => h.fecha), datasets: [{ label: 'Kcal', data: dataH.map(h => h.cardio.kcal), backgroundColor: '#FF2D55', order: 2 }, { label: 'Min', data: dataH.map(h => h.cardio.min), borderColor: '#FFF', type: 'line', order: 1, tension: 0.3 }] }, options: { plugins: { legend: { labels: { color: '#8e8e93', font: { size: 10 } } } } } });
}

function updateChartRendimiento() {
    const ejNome = document.getElementById('select-ejercicio-grafica').value;
    if(!ejNome) return;
    const filtrados = window.db.historial.filter(h => h.ejercicios.some(e => e.nome === ejNome));
    const datos = filtrados.map(h => ({ f: h.fecha, v: Math.max(...h.ejercicios.find(e => e.nome === ejNome).series.map(s => s.kg)) }));
    const ctx = document.getElementById('graficaRendimiento').getContext('2d');
    if(chartRendimiento) chartRendimiento.destroy();
    chartRendimiento = new Chart(ctx, { type: 'bar', data: { labels: datos.map(d => d.f), datasets: [{ label: 'Kg Max', data: datos.map(d => d.v), backgroundColor: '#30D158' }] }, options: { plugins: { legend: { display: false } } } });
}
...
function verDetalleHistorial(idx) {
    ...
    h.ejercicios.forEach(e => {
        html += `<h4>${e.nome}</h4><table style="width:100%; font-size:13px">` + e.series.map(s => `<tr><td style="color:gray">Serie ${s.n}</td><td>${s.kg}kg</td><td>${s.reps} reps</td></tr>`).join('') + `</table><p style="color:gray; font-size:11px">${e.nota || ''}</p>`;
    });
...
function registrarPesoCorporal() {
    ...
    if(p) { window.db.medidas.unshift({ fecha: new Date().toLocaleDateString(), valor: parseFloat(p) }); save(); }
}
